    ...
    switch (_connectionState)
    {
          ...
          if (_parserLine.parsingFinished && _connectionState != STATE_ERROR)
          {
            ...
            else
            {
              ...
              if ( (idxColon != std::string::npos) && (_parserLine.text[idxColon + 1] == ' ') )
              {
                ...
                HTTPS_LOGD("Header: %s = %s (FID=%d)", _parserLine.text.substr(0, idxColon).c_str(),
                           _parserLine.text.substr(idxColon + 2).c_str(), _socket);
              }
              else
              {
      ...
      case STATE_HEADERS_FINISHED: // Handle body
      {
        ...
        _resResolver->resolveNode(_httpMethod, _httpResource, resolvedResource,
                                  websocketRequested ? WEBSOCKET : HANDLER_CALLBACK);
        ...
        if (resolvedResource.didMatch())
        {
          ...
          for (std::vector<HTTPHeader*>::iterator header = allDefaultHeaders->begin(); header != allDefaultHeaders->end();
               ++header)
          {
...
void validationMiddleware(HTTPRequest * req, HTTPResponse * res, std::function<void()> next)
{
  ...
  for (std::vector<HTTPValidator*>::iterator validator = validators->begin(); valid
       && validator != validators->end(); ++validator)
  {
    std::string param;
